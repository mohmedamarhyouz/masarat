...
export function LifePage() {
  ...
  return (
    <div className="page life-page">
      ...
      {selectedArea && <motion.section key={selectedArea.id} className="selected-area-panel" style={{ '--area': selectedArea.color } as React.CSSProperties} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}><div className="selected-area-panel__identity"><span style={{ background: selectedArea.color }} /><div><small>{t.lifeOrbit}</small><h3>{selectedArea.name}</h3></div><b className={`area-status area-status--${selectedArea.status}`}><i />{statusLabels[selectedArea.status]}</b></div><div className="selected-area-panel__stats"><span><strong>{selectedGoals.length}</strong>{t.areaGoals}</span><span><strong>{selectedProjects.length}</strong>{t.areaPaths}</span><span><strong>{selectedProjects.flatMap((project) => project.tasks).filter((task) => task.status === 'completed').length}</strong>{t.completed}</span></div></motion.section>}
