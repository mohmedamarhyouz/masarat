  ...
  return (
    <div className="page path-overview-page">
      ...
      <div className="path-overview-layout">
        ...
        <aside className="decision-context"><h3><ShieldCheck size={18} /> {c.context}</h3><div className="path-link-fields"><label><small>{c.area}</small><select value={project.project.areaId ?? 'area-uncategorized'} onChange={(event) => updateProjectMeta(project.project.id, { areaId: event.target.value, goalId: undefined })}>{lifeAreas.filter((item) => !item.archived).map((item) => <option key={item.id} value={item.id}>{localizedAreaName(item, language)}</option>)}</select></label><label><small>{c.linkedGoal}</small><select value={project.project.goalId ?? ''} onChange={(event) => updateProjectMeta(project.project.id, { goalId: event.target.value || undefined })}><option value="">{c.noGoal}</option>{goals.filter((item) => item.areaId === (project.project.areaId ?? 'area-uncategorized')).map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label></div><div><small>{c.why}</small><p>{project.project.goal}</p></div><div><small>{c.constraints}</small><ul>{project.constraints.slice(0, 4).map((constraint) => <li key={constraint.id}>{constraint.title}</li>)}</ul></div><button className="text-action" onClick={() => setView('canvas')}>{c.explore} <ArrowLeft size={14} /></button></aside>
      </div>
    </div>
  )
}
